...
\begin{figure}[htbp]
    \caption{\small Exemplo de uma Árvore de Huffman} 
    \centering
    \includegraphics[width=1\textwidth]{figs/huffman_tree.png}
...
O algoritmo LZ77, proposto por Abraham Lempel e Jacob Ziv em 1977~\cite{ziv1977universal}, representa um marco na compressão de dados sem perdas ao introduzir o conceito de compressão baseada em dicionário dinâmico. Diferentemente dos métodos estatísticos como Huffman, que dependem da frequência de símbolos individuais, o LZ77 explora a redundância temporal presente em sequências de dados, identificando e codificando repetições de subsequências já observadas.
...
\begin{itemize}
    \item \textbf{Buffer de busca}: região que contém os dados já processados, servindo como dicionário para identificar repetições;
    \item \textbf{Buffer de antecipação}: região que contém os dados ainda não processados, onde se busca por correspondências com o buffer de busca.
\end{itemize}

O algoritmo funciona através dos seguintes passos:

\begin{enumerate}
    \item \textbf{Inicialização}: Define-se o tamanho do buffer de busca tipicamente entre 4KB e 32KB e do buffer de antecipação entre 4 e 258 bytes;
    
    \item \textbf{Busca por correspondências}: Para cada posição no buffer de antecipação, procura-se a maior subsequência que tenha uma correspondência no buffer de busca;
    
    \item \textbf{Codificação}: Se uma correspondência é encontrada, ela é codificada como um triplo $(\text{distância}, \text{comprimento}, \text{próximo\_símbolo})$, onde:
    \begin{itemize}
        \item \textit{distância}: número de posições para trás onde a correspondência foi encontrada;
        \item \textit{comprimento}: número de símbolos que correspondem;
        \item \textit{próximo\_símbolo}: primeiro símbolo após a correspondência que não faz parte dela.
    \end{itemize}
    
    \item \textbf{Avanço da janela}: A janela desliza para a direita, incorporando os símbolos processados ao buffer de busca e carregando novos símbolos no buffer de antecipação;
    
    \item \textbf{Repetição}: O processo continua até que todos os dados sejam processados.
\end{enumerate}

\subsubsection{Exemplo prático}

Considere a sequência de entrada: \texttt{"AACAACABCABAAAC"}

Assumindo uma janela de busca de tamanho 6 e buffer de antecipação de tamanho 4:
...
\texttt{(0,0,A); (1,1,C); (3,4,B); (3,3,A); (1,2,C)}

\subsubsection{Variações e otimizações}

O algoritmo LZ77 possui diversas variações que melhoram seu desempenho:

\begin{itemize}
    \item \textbf{LZSS (Lempel-Ziv-Storer-Szymanski)}: Elimina o campo "próximo\_símbolo" do triplo, usando um bit de flag para distinguir entre literais e referências, resultando em melhor compressão;
    
    \item \textbf{LZ77 com hash}: Utiliza tabelas hash para acelerar a busca por correspondências, reduzindo a complexidade de $O(n \cdot w)$ para $O(n)$, onde $w$ é o tamanho da janela;
    
    \item \textbf{LZ77 com árvores de sufixos}: Emprega estruturas de dados mais sofisticadas para otimizar a busca, especialmente eficaz para textos longos;
    
    \item \textbf{LZ77 com codificação aritmética}: Combina o LZ77 com codificação aritmética em vez de representação direta dos triplos, melhorando a taxa de compressão.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Implementação ingênua}: $O(n \cdot w \cdot l)$, onde $n$ é o tamanho da entrada, $w$ o tamanho da janela de busca e $l$ o comprimento máximo de correspondência;
    
    \item \textbf{Com otimizações (hash)}: $O(n \cdot l)$ no caso médio, $O(n \cdot w \cdot l)$ no pior caso;
    
    \item \textbf{Espaço}: $O(w + l)$ para armazenar as janelas de busca e antecipação.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Universalidade}: É um algoritmo universal, ou seja, para qualquer fonte estacionária e ergódica, a taxa de compressão converge para a taxa de entropia da fonte quando o tamanho da janela tende ao infinito;
    
    \item \textbf{Adaptabilidade}: Não requer conhecimento prévio das estatísticas da fonte, adaptando-se dinamicamente aos padrões presentes nos dados;
    
    \item \textbf{Garantia de não expansão}: No pior caso, o tamanho da saída é limitado pelo tamanho da entrada mais um overhead constante por bloco processado.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tamanho do buffer de busca}: Janelas maiores permitem encontrar correspondências mais distantes, mas aumentam o tempo de processamento e o espaço necessário para codificar as distâncias;
    
    \item \textbf{Comprimento máximo de correspondência}: Valores maiores permitem codificar repetições mais longas, mas requerem mais bits para representar o comprimento;
    
    \item \textbf{Comprimento mínimo de correspondência}: Correspondências muito curtas podem não compensar o overhead da codificação, sendo mais eficiente tratá-las como literais.
\end{itemize}

\textbf{Vantagens}:
\begin{itemize}
    \item Algoritmo universal que não requer conhecimento prévio das estatísticas da fonte;
    \item Adapta-se dinamicamente aos padrões presentes nos dados;
    \item Excelente desempenho em dados com repetições estruturais (textos, código fonte, dados binários com padrões);
    \item Descompressão rápida e de baixa complexidade;
    \item Base teórica sólida com garantias de convergência para a taxa de entropia.
\end{itemize}

\textbf{Limitações}:
\begin{itemize}
    \item Eficiência limitada pelo tamanho da janela de busca, que determina quão distantes podem estar as repetições;
    \item Desempenho inferior em dados altamente aleatórios ou sem padrões repetitivos;
    \item Overhead significativo para representar as referências quando as correspondências são curtas;
    \item Complexidade computacional elevada na compressão, especialmente sem otimizações;
    \item Sensibilidade à escolha dos parâmetros (tamanhos de janela e buffer).
\end{itemize}

\textbf{Casos de uso}:
\begin{itemize}
    \item Base fundamental do algoritmo DEFLATE, usado em formatos ZIP, PNG e HTTP/HTTPS (gzip);
    \item Compressão de arquivos de texto, código fonte e documentos estruturados;
    \item Sistemas de backup e arquivamento que priorizam alta taxa de compressão;
    \item Protocolos de comunicação que requerem compressão em tempo real com boa adaptabilidade;
    \item Compressão de dados binários com padrões repetitivos (executáveis, bibliotecas).
\end{itemize}
