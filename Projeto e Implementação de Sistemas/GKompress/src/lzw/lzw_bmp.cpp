...
static void read_bmp_header(FILE* file, void* image_file) {
    ...
    if (bmp_ptr->has_color_table()) {
        ...
        fread(bmp_ptr->color_table, color_table_size, 1, file);
    }
}
...
void LZW_BMP::compress(int option) {
    bmp_file.read(filepath.c_str());
    std::vector<std::pair<uint16_t, int>> codes = encode(bmp_file.data, bmp_file.data_size);
    internal_compress(option, codes, write_bmp_header, &bmp_file);
}

int LZW_BMP::decompress(int option) {
    bmp decompressed_bmp;
    return internal_decompress(option, write_bmp_header, read_bmp_header, get_pos, &decompressed_bmp);
}