...
def plot_compression_scatter(csv_file, file_extension):
    ...
    plt.scatter(sizes_in_kb, compression_rates, color='mediumseagreen', alpha=0.6)

    # Adicionando título e rótulos
    plt.title(f"Gráfico de Dispersão: Tamanho do Arquivo vs Taxa de Compressão\n({file_extension.upper()})")
    plt.xlabel("Tamanho do Arquivo (KB)")
    plt.ylabel("Taxa de Compressão (%)")
    plt.grid(True, linestyle='--', alpha=0.6)

    # Salvando o gráfico
    output_image = f"{os.path.splitext(csv_file)[0]}_{file_extension}_scatter_plot.png"
    plt.tight_layout()
    plt.savefig(output_image)
    plt.close()

    print(f"Gráfico de dispersão salvo: {output_image}")

def generate_all_scatter_plots(csv_folder='.'):
    for file in os.listdir(csv_folder):
        if file.endswith('.csv') and file.startswith('file_sizes'):
            for extension in ['txt', 'wav', 'bmp']:
                plot_compression_scatter(os.path.join(csv_folder, file), extension)

# Rodar a função para gerar os gráficos de dispersão
