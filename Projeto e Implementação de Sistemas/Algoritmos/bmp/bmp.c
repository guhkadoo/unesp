...
int read_bmp(const char* filename, bmp* file_data)
{
    ...
    if(file_data->info_header.bits_per_pixel > 8)
    {
        file_data->color_table = NULL;
    } else {
        size_t color_table_size = 4 * (1 << file_data->info_header.bits_per_pixel);
    ...
    printf("teste\nfile_size = %d\ninfo_header_size: %d\ndata_size: %d\n", file_data->header.file_size, file_data->info_header.size, data_size); 
