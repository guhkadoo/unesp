...
{
    std::string filename = get_filename_with_ext(filepath);
    filename.append(".GK");
...
void Huffman::decompress(void (*write_header)(FILE*, void*), void* filetype, size_t pos) 
{
    ...
    filename.append(get_extension(filepath));

    std::string filename_compressed = get_filename_with_ext(filepath);
    filename_compressed.append(".GK");
